...
import { LOCATION_DATA, POPULAR_CITIES } from '../constants';
...
const CitySelect: React.FC<CitySelectProps> = ({ currentCity, onSelect, onBack }) => {
    ...
    const getCurrentLevelData = () => {
        if (path.length === 0) return Object.keys(LOCATION_DATA); // Province List
        if (path.length === 1) return Object.keys(LOCATION_DATA[path[0]] || {}); // City List
        if (path.length === 2) return Object.keys(LOCATION_DATA[path[0]][path[1]] || {}); // District List
        if (path.length === 3) return LOCATION_DATA[path[0]][path[1]][path[2]] || []; // Town List
        return [];
    };
    ...
    const handleItemClick = (item: string) => {
        ...
        // In this mock, Town is the last level (path length becomes 4)
        if (path.length === 3) {
